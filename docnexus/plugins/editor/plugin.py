...
def get_source(filename):
    ...
    logger.debug(f"Handling get-source request for: {filename}")
    ...
    try:
        ...
        logger.info(f"Served source for: {filename} ({len(content)} bytes)")
        return jsonify({'content': content, 'path': str(filename)})
    
    except Exception as e:
        logger.error(f"Error reading source {filename}: {e}", exc_info=True)
